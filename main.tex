...
\author{%
	\IEEEauthorblockN{\textit{1\textsuperscript{st} Maulana Anjari Anggorokasih}}
	\IEEEauthorblockA{%
	...
	\and
	\IEEEauthorblockN{\textit{2\textsuperscript{nd} Noor Akhmad Setiawan}}
	\IEEEauthorblockA{%
		\textit{Department of Electrical and}\\
		\textit{Information Engineering}\\
		\textit{Faculty of Engineering}\\
		...
		noorwewe@ugm.ac.id}
	\and
	\IEEEauthorblockN{\textit{3\textsuperscript{rd} Teguh Bharata Adji}}
	\IEEEauthorblockA{%
		\textit{Department of Electrical and}\\
		\textit{Information Engineering}\\
		\textit{Faculty of Engineering}\\
...
The technical implementation uses a combination of existing open-source software (given) and components developed by the researcher. Given software includes Geth, Prysm, Kurtosis, Hyperledger Caliper, and Kubernetes. Self-developed components include bash and Docker-based PoA network generation scripts, Starlark configuration for PoS network orchestration, automated benchmarking pipeline (\texttt{run\_pipeline.sh}), and centralized monitoring system configuration with Dual-Mode Scraping mechanism. All source code is publicly available at the repository listed in the Acknowledgment section.

\subsection{Network Topology}

The experimental topology employs a 10-node configuration distributed across 6 VPS instances, as illustrated in Figure \ref{fig:testbed_architecture}. The infrastructure consists of:
\begin{itemize}
    \item \textbf{Control Plane VPS}: Orchestration center running Kubernetes Master, Docker, Prometheus (metrics collector), Grafana (visualization), Ethstats (Ethereum network monitoring), Kurtosis (PoS orchestration), PostgreSQL (data storage), and Hyperledger Caliper (transaction load generator)
    \item \textbf{Validator Nodes}: 5 nodes (black border) with rights to validate transactions and produce new blocks. In PoA consensus, these nodes use Round-Robin proposer rotation; in PoS, validator selection is randomized
    \item \textbf{Observer Nodes}: 5 nodes (red border) that participate in data synchronization without validation rights, functioning as RPC gateways for transactions sent by Caliper
\end{itemize}

Each worker VPS runs Kubernetes Worker, Pushgateway, Node Exporter, and cAdvisor to collect performance metrics forwarded to Prometheus on the Control Plane. The orange dashed arrows indicate RPC Load flow from Hyperledger Caliper to Observer Nodes for benchmarking tests.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.95\columnwidth]{testbed_architecture.png}
    \caption{Testbed Infrastructure Architecture: Distribution of Software Components Across VPS Instances}
    \label{fig:testbed_architecture}
\end{figure}

\subsection{Benchmarking Scenarios}

Six main scenarios were applied to test various performance aspects following the 6-Layer Benchmarking Strategy:

\begin{enumerate}
    \item \textbf{Baseline Performance}: Measure intrinsic system latency under minimal load to establish baseline metrics
    \item \textbf{Stress Testing (Step Load)}: Find system saturation point (knee point) by gradually increasing TPS load (5, 10, ..., 100 TPS) until latency spikes drastically
    \item \textbf{Scalability Testing}: Test concurrent client handling efficiency by running optimal TPS load with variations of 1, 3, and 5 workers, and measure read operation (RPC) performance with high read loads (200, 400, 600 TPS)
    \item \textbf{Resource Efficiency}: Measure computational resource usage (CPU, RAM, Disk I/O, Network) to evaluate infrastructure efficiency
    \item \textbf{Resilience \& Stability (Soak Test)}: Test long-term stability through Soak Test for 15 minutes at 60\% optimal load to detect cumulative performance degradation
    \item \textbf{Functional Lifecycle}: Validate data consistency through complete transaction lifecycle (Mint-Read-Burn)
\end{enumerate}
...
The high-level architecture of Hyperledger Caliper used in this research is illustrated in Figure \ref{fig:caliper_architecture}. The architecture shows the workflow from configuration inputs (Workload Module, Benchmark Configuration, Network Configuration) through the Caliper engine to the System Under Test (SUT), producing benchmark reports as output \cite{hyperledger_caliper}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\columnwidth]{caliper_architecture.png}
    \caption{High-level Architecture of Hyperledger Caliper (Source: Official Documentation)}
    \label{fig:caliper_architecture}
\end{figure}
...
Throughput Step testing increased load from 5 to 100 TPS. Results showed that in 10-node topology, PoA began experiencing saturation at load $\approx$30--40 TPS, while PoS maintained linearity up to $\approx$60 TPS before saturation (see Figure \ref{fig:throughput_curve}). PoS achieved peak throughput higher than PoA in this configuration, with the comparison of optimal stability points summarized in Figure \ref{fig:optimal_tps}.
...
Latency measurement was conducted under steady-state conditions. The latency trade-off graph (Figure \ref{fig:latency_tradeoff}) shows the relationship between load (TPS) and response time.
\begin{itemize}
...
PoA shows clear superiority in responsiveness aspect. PoA average latency (4.6 seconds) is very close to block time (5 seconds), showing minimal inefficiency. Conversely, PoS (8 seconds) suffers from inherent latency from slot mechanism and initial probabilistic finality. For academic service systems, this 3--4 second difference is significant in terms of UX (User Experience).
...
In summary, the comprehensive benchmarking results provide a clear technical justification for DChain's strategic consensus roadmap. By filling the research gaps with geographically distributed testing and functionally relevant workloads, this study establishes that while PoS offers superior raw throughput, PoA's latency and resource efficiency are more aligned with the immediate needs of academic certificate verification. This empirical foundation ensures that any future protocol migration within the national consortium can be executed with a precise understanding of the performance trade-offs, ultimately sustaining the quality of digital services in the Indonesian higher education ecosystem.
