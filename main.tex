...
Throughput Step testing increased load from 5 to 100 TPS. Results showed that in 3-node topology, PoA began experiencing saturation at load $\approx$30 TPS, while PoS maintained linearity up to $\approx$60 TPS before saturation. PoS achieved peak throughput 2× higher than PoA in this configuration.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{throughput_curve_3_Nodes.png}
    \caption{Throughput Curve on 3-Node Topology}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\columnwidth]{latency_tradeoff_3_Nodes.png}
...
Data shows an interesting phenomenon where PoS actually achieves 2× higher saturation throughput than PoA at small scale (3 Nodes). This is suspected to be caused by Block Time limitations. PoA with 5-second blocks has a physical limit on gas amount/block per time unit. Conversely, PoS with 12-second slots, although the interval is longer, allows more efficient transaction batching with the same gas limit per block, producing higher aggregate throughput before RPC saturation.
